  ...
  <div class="row row-xs">
    <div class="col-lg-12 col-xl-12 mg-t-0">
      <div class="card card-accent-green-700 shadow-sm">
        <div class="card-body">
          <form method="POST" id="formEdit" name="formEdit" action="{{route('school_cycles.update', $schoolCycle)}}" class="bd bd-1 rounded">
            @csrf
            @method('PATCH')
  ...
  <script>
    $().ready( function () {
      ...
      $('#formEdit').parsley({
        ...
        submitForm("POST","{{ route('school_cycles.update', $schoolCycle->id) }}", $("#formEdit").serialize());
      });
