    ...
    <div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-style1 mg-b-10">
          ...
          <li class="breadcrumb-item active">Nuevo Grado Escolar</li>
        </ol>
      </nav>
      <h4 class="mg-b-0 tx-spacing--1">Nuevo Grado Escolar</h4>
    </div>
  ...
  <div class="row row-xs">
    <div class="col-lg-12 col-xl-12 mg-t-0">
      <div class="card card-accent-green-700 shadow-sm">
        <div class="card-body">
          <form method="POST" id="formCreate" name="formCreate" action="{{route('school_grades.store')}}" class="bd bd-1 rounded">
            @csrf
...
<script>
  $().ready( function () {
    ...
    $('#formCreate').parsley({
      ...
      submitForm("POST","{{ route('school_grades.store') }}", $("#formCreate").serialize());
    });
