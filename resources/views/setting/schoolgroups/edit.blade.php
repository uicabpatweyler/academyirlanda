    ...
    <div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-style1 mg-b-10">
          ...
          <li class="breadcrumb-item active">Editar Grupo Escolar</li>
        </ol>
      </nav>
      <h4 class="mg-b-0 tx-spacing--1">Editar Grupo Escolar</h4>
    </div>
  ...
  <div class="row row-xs">
    <div class="col-lg-12 col-xl-12 mg-t-0">
      <div class="card card-accent-green-700 shadow-sm">
        <div class="card-body">
          <form method="POST" id="formEdit" name="formEdit" action="{{route('school_groups.update', $schoolGroup)}}" class="bd bd-1 rounded">
            @csrf
            @method('PATCH')
              ...
              <div class="form-group col-sm-4">
                ...
                <select name="school_id" id="school_id"
                  ...
                  @foreach($schools as $school)
                    ...
                    @endif
                    <option value="{{$school->id}}" {{$school->id === $schoolGroup->school_id ? 'selected' : ''}}>{{$school->name}}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-sm-4 d-flex flex-column align-items-start">
                ...
                <select name="school_cycle_id" id="school_cycle_id"
                  ...
                  @foreach($cycles as $cycle)
                    ...
                    @endif
                    <option value="{{$cycle->id}}" {{$schoolGroup->school_cycle_id === $cycle->id ? 'selected' : ''}}>{{$cycle->period}}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-sm-4 d-flex flex-column align-items-start">
                ...
                <select name="school_grade_id" id="school_grade_id"
                  ...
                  @foreach($schoolGrades as $schoolGrade)
                    ...
                    @endif
                    <option value="{{$schoolGrade->id}}" {{$schoolGrade->id === $schoolGroup->school_grade_id ? 'selected' : ''}}>{{$schoolGrade->name}}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="row row-sm pd-x-20 pd-b-5">
              <div class="form-group col-sm-5">
                ...
                <select name="fee_one" id="fee_one"
                        ...
                        required>
                  @foreach($schoolFeeOne as  $schoolFee)
                    @if($loop->first)
                      <option value="">[Elija una cuota]</option>
                    @endif
                      <option value="{{$schoolFee->id}}" {{$schoolGroup->fee_one === $schoolFee->id ? 'selected' : ''}}>
                        {{$schoolFee->name}} ( {{$schoolFee->amount_format}} )
                      </option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-sm-5 d-flex flex-column align-items-start">
                ...
                <select name="fee_two" id="fee_two"
                        ...
                        required>
                  @foreach($schoolFeeTwo as  $schoolFee)
                    @if($loop->first)
                      <option value="">[Elija una cuota]</option>
                    @endif
                    <option value="{{$schoolFee->id}}" {{$schoolGroup->fee_two === $schoolFee->id ? 'selected' : ''}}>
                      {{$schoolFee->name}} ( {{$schoolFee->amount_format}} )
                    </option>
                  @endforeach
                </select>
            ...
            <div class="row row-sm pd-x-20 pd-b-5">
              <div class="form-group col-sm-4">
                ...
                <input type="text"  class="form-control @error('name') is-invalid @enderror"
                       ...
                       id="name" name="name" autocomplete="name" minlength="3"
                       value="{{$schoolGroup->name}}" required>
              </div>
              <div class="form-group col-sm-4 d-flex flex-column align-items-start">
                ...
                <input type="text"  class="form-control @error('allowed_students') is-invalid @enderror"
                       ...
                       id="allowed_students" name="allowed_students" autocomplete="allowed_students"
                       value="{{$schoolGroup->allowed_students}}" required>
              </div>
            </div>
...
$().ready( function () {
  ...
  $('#formEdit').parsley({
    ...
    submitForm("POST","{{ route('school_groups.update', $schoolGroup) }}", $("#formEdit").serialize());
  });
